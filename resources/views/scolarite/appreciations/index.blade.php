<x-scolarite-layout>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="bg-white rounded-lg shadow-sm p-5 h-fit">
            ...
            <form method="POST" action="{{ route('scolarite.appreciations.store') }}" class="space-y-3">
                @csrf
                <div>
                    ...
                    <select name="matricule" required class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                        ...
                        @foreach ($eleves as $el)
                            <option value="{{ $el->matricule }}" @selected(old('matricule') == $el->matricule)>{{ $el->nom }} {{ $el->prenom }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="idTrimes" required class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                        ...
                        @foreach ($trimestres as $t)
                            <option value="{{ $t->idTrimes }}" @selected(old('idTrimes') == $t->idTrimes)>{{ $t->libelle }}{{ $t->annee ? ' — ' . $t->annee->libelle : '' }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <button class="w-full py-2 bg-amber-800 text-white text-sm rounded-md hover:bg-amber-900">Enregistrer</button>
                        ...
                        @forelse ($appreciations as $a)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <form method="POST" action="{{ route('scolarite.appreciations.destroy', $a) }}" class="inline"
