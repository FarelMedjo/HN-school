<x-scolarite-layout>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="bg-white rounded-lg shadow-sm p-5 h-fit">
            ...
            <form method="POST" action="{{ route('scolarite.remarques.store') }}" class="space-y-3">
                @csrf
                <div>
                    ...
                    <select name="matricule" required class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                        ...
                        @foreach ($eleves as $el)
                            <option value="{{ $el->matricule }}" @selected(old('matricule', $matricule) == $el->matricule)>{{ $el->nom }} {{ $el->prenom }}</option>
                        @endforeach
                    </select>
                </div>
                        ...
                        @forelse ($remarques as $r)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <form method="POST" action="{{ route('scolarite.remarques.destroy', $r) }}" class="inline"
