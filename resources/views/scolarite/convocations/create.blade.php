<x-scolarite-layout>
    ...
    <form method="POST" action="{{ route('scolarite.convocations.store') }}" class="bg-white p-6 rounded-lg shadow-sm space-y-4 max-w-2xl">
        @csrf
        <div>
        ...
        <div class="flex gap-3">
            <button class="px-4 py-2 bg-amber-800 text-white rounded-md hover:bg-amber-900">Enregistrer & imprimer</button>
            <a href="{{ route('scolarite.convocations.index') }}" class="px-4 py-2 text-gray-600 hover:text-gray-800">Annuler</a>
        </div>
    </form>
</x-scolarite-layout>
