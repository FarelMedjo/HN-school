<x-admin-layout>
    <x-page-header title="Convocations" :createRoute="route('admin.convocations.create')" createLabel="Nouvelle convocation" />
    ...
    <form method="GET" class="bg-white rounded-lg shadow-sm p-4 mb-4 flex flex-wrap items-end gap-4">
        <div class="flex-1 min-w-[220px]">
            ...
            <select name="matricule" onchange="this.form.submit()"
                ...
                @foreach ($eleves as $el)
                    <option value="{{ $el->matricule }}" @selected($matricule == $el->matricule)>
                        {{ $el->nom }} {{ $el->prenom }}
                    </option>
                @endforeach
            </select>
        ...
        @if ($matricule)
            <a href="{{ route('admin.convocations.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Réinitialiser</a>
        @endif
    </form>
                ...
                @forelse ($convocations as $c)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                            <a href="{{ route('admin.convocations.show', $c) }}" target="_blank" class="text-blue-600 hover:underline">Imprimer</a>
                            <form method="POST" action="{{ route('admin.convocations.destroy', $c) }}" class="inline"
